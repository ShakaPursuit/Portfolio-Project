...
const getArtistById= async(id) =>{
    try{ 
        const artist = await db.one('SELECT * FROM artist WHERE id=$1',[id])
            return artist;
...
const updateArtist = async (artist, id) => {
    try {
        const updatedArtist = await db.one('UPDATE artist SET name=$1,skills=$2,genre=$3,instruments=$4,independent=$5,years_of_experience=$6,record_label=$7 where id=$8 RETURNING * ', [artist.name, artist.skills, artist.genre, artist.instruments, artist.independent, artist.years_of_experience, artist.record_label, id]);
